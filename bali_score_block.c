...
int main(int argc, char **argv)
{
	...
	if(argc!=3) {
		...
		fprintf(stderr,"                where ref_aln       reference alignment in xml/fasta format \n");
		fprintf(stderr,"                      test_aln      test alignment in msf/fasta format \n");
			return 1;
	}
...
/* read the test alignment into testaln */
	...
	if(nseqs==0) {
		fprintf(stderr,"Error: no sequences in test alignment %s\n",argv[2]);
		return 1;
	}
...
/* calculate percentage of block prdicted to be disordered */
		...
		fprintf(stdout,"BCS %d %s %d %d %d %d %.2f %d %.2f\n",n,ref_aln.seqs[block[n].seq[0]].name,start_col,end_col,block[n].len,pcseqs,md,ndisorder,block_col_score);
